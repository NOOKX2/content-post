"use client";

import { ChevronLeft, ChevronRight, LayoutGrid, List, MessageCircle, Plus } from "lucide-react";
import { useCallback, useState } from "react";
import { useTeamCalendarWorkspace } from "@/app/collaboration/_hooks/use-team-calendar-workspace";
import { PersonAvatar } from "@/app/collaboration/_components/PersonAvatar";
import { TeamWeekGrid } from "@/app/collaboration/_components/TeamWeekGrid";
import { TeamMonthGrid } from "@/app/collaboration/_components/TeamMonthGrid";
import { NewMeetingPanel } from "@/app/collaboration/_components/NewMeetingPanel";
import { firstName, formatWeekRange } from "@/app/collaboration/_lib/calendar-utils";
import { cn } from "@/lib/shared/utils";

export function TeamCalendarWorkspace({
  selectedMemberId,
  onSelectMember,
  onMessageMember,
}: {
  selectedMemberId: string | null;
  onSelectMember: (userId: string) => void;
  onMessageMember: (userId: string) => void;
}) {
  const {
    members,
    selected,
    meetings,
    isLoading,
    meetingsByMemberId,
    defaultAttendeeIds,
    view, setView,
    anchor,
    weekStart,
    selectedDate,
    monthName,
    eventsThisMonth,
    isMobile,
    panelOpen, setPanelOpen,
    prefillStart, prefillEnd,
    submitting,
    currentUserId,
    isSelf,
    navigate,
    openPanelFor,
    updatePanelRange,
    clearPanelRange,
    handleSubmit,
    t, loc,
    roleLabel,
  } = useTeamCalendarWorkspace(selectedMemberId, onSelectMember);

  const [draftPreview, setDraftPreview] = useState<{
    start: Date;
    end: Date;
    title: string;
  } | null>(null);

  const handleDraftRangeChange = useCallback(
    (range: { start: Date; end: Date; title: string }) => {
      setDraftPreview((prev) => {
        if (
          prev &&
          prev.start.getTime() === range.start.getTime() &&
          prev.end.getTime() === range.end.getTime() &&
          prev.title === range.title
        ) {
          return prev;
        }
        return range;
      });
    },
    []
  );

  const handleSelectSlot = useCallback(
    (range: { start: Date; end: Date } | null) => {
      if (!panelOpen) return;
      if (!range) {
        clearPanelRange();
        setDraftPreview(null);
        return;
      }
      updatePanelRange(range.start, range.end);
      setDraftPreview((prev) => ({
        start: range.start,
        end: range.end,
        title: prev?.title ?? "",
      }));
    },
    [panelOpen, updatePanelRange, clearPanelRange]
  );

  const closePanel = () => {
    setPanelOpen(false);
    setDraftPreview(null);
  };

  if (!selected) {
    return (
      <div className="flex flex-1 items-center justify-center text-sm text-stone-500">
        {t("team.noMembers")}
      </div>
    );
  }

  const showPanel = panelOpen && !isMobile;

  return (
    <div className="flex min-h-0 min-w-0 flex-1 bg-white">
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        {/* Top toolbar */}
        <div className="grid items-center gap-3 border-b border-stone-200/80 bg-white px-4 py-3 lg:grid-cols-[1fr_auto_1fr]">
          <div className="flex min-w-0 items-center gap-2.5 overflow-x-auto py-2 pl-2.5 pr-1">
            {members.map((member) => {
              const active = member.id === selected.id;
              return (
                <button
                  key={member.id}
                  type="button"
                  title={member.name}
                  onClick={() => onSelectMember(member.id)}
                  className={cn(
                    "shrink-0 rounded-full p-0.5 transition",
                    active
                      ? "ring-2 ring-blue-500 ring-offset-2 ring-offset-white"
                      : "opacity-90 hover:opacity-100"
                  )}
                >
                  <PersonAvatar
                    name={member.name}
                    imageUrl={member.imageUrl}
                    size="lg"
                    letters={2}
                    className="ring-0!"
                  />
                </button>
              );
            })}
          </div>

          <div className="flex items-center justify-center gap-1">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="rounded-full p-1.5 text-stone-500 transition hover:bg-stone-100 hover:text-stone-800"
              aria-label={t("common.back")}
            >
              <ChevronLeft className="h-4 w-4" />
            </button>
            <span className="min-w-44 text-center text-sm font-bold text-stone-900">
              {view === "week"
                ? formatWeekRange(weekStart, loc)
                : anchor.toLocaleDateString(loc, { month: "long", year: "numeric" })}
            </span>
            <button
              type="button"
              onClick={() => navigate(1)}
              className="rounded-full p-1.5 text-stone-500 transition hover:bg-stone-100 hover:text-stone-800"
            >
              <ChevronRight className="h-4 w-4" />
            </button>
          </div>

          <div className="flex flex-wrap items-center justify-end gap-2">
            <div className="flex rounded-lg bg-stone-100/90 p-1">
              {(["week", "month"] as const).map((item) => (
                <button
                  key={item}
                  type="button"
                  onClick={() => setView(item)}
                  className={cn(
                    "inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-semibold transition",
                    view === item
                      ? "bg-white text-stone-900 shadow-sm"
                      : "text-stone-500 hover:text-stone-800"
                  )}
                >
                  {item === "week" ? (
                    <List className="h-3.5 w-3.5" />
                  ) : (
                    <LayoutGrid className="h-3.5 w-3.5" />
                  )}
                  {item === "week" ? t("team.week") : t("team.month")}
                </button>
              ))}
            </div>

            {!panelOpen ? (
              <button
                type="button"
                onClick={() => {
                  const start = new Date(selectedDate);
                  start.setHours(10, 0, 0, 0);
                  const end = new Date(start);
                  end.setMinutes(end.getMinutes() + 60);
                  openPanelFor(start, end);
                  setDraftPreview({ start, end, title: "" });
                }}
                className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-blue-600 px-3.5 text-xs font-semibold text-white shadow-sm shadow-blue-600/20 transition hover:bg-blue-700"
              >
                <Plus className="h-3.5 w-3.5" />
                {t("team.newMeetingAction")}
              </button>
            ) : null}
          </div>
        </div>

        {/* Member sub-header */}
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200/80 bg-white px-5 py-3.5">
          <div className="flex min-w-0 flex-wrap items-center gap-2.5">
            <h2 className="truncate text-lg font-bold text-stone-900">
              {selected.name}
            </h2>
            <span className="text-sm text-stone-500">
              {roleLabel(selected)}
            </span>
            <span
              className={cn(
                "inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold",
                selected.busy
                  ? "bg-amber-50 text-amber-700"
                  : "bg-emerald-50 text-emerald-700"
              )}
            >
              <span
                className={cn(
                  "h-1.5 w-1.5 rounded-full",
                  selected.busy ? "bg-amber-500" : "bg-emerald-500"
                )}
              />
              {selected.busy ? t("team.busyNow") : t("team.online")}
            </span>
          </div>
          <div className="flex shrink-0 items-center gap-3">
            {!isSelf ? (
              <button
                type="button"
                onClick={() => onMessageMember(selected.id)}
                className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-3.5 py-1.5 text-xs font-semibold text-stone-700 transition hover:bg-stone-50"
              >
                <MessageCircle className="h-3.5 w-3.5" />
                {t("team.messageMember", { name: firstName(selected.name) })}
              </button>
            ) : null}
            <span className="text-sm font-medium text-stone-500">
              {t("team.eventsInMonth", { count: eventsThisMonth, month: monthName })}
            </span>
          </div>
        </div>

        {isLoading ? (
          <p className="py-16 text-center text-sm text-stone-500">{t("common.loading")}</p>
        ) : view === "week" ? (
          <TeamWeekGrid
            weekStart={weekStart}
            meetings={meetings}
            selectedDate={
              panelOpen && draftPreview ? draftPreview.start : selectedDate
            }
            previewRange={panelOpen ? draftPreview : null}
            onSelectSlot={handleSelectSlot}
          />
        ) : (
          <TeamMonthGrid
            year={anchor.getFullYear()}
            month={anchor.getMonth()}
            meetings={meetings}
            selectedDate={selectedDate}
            onSelectDate={() => {
              /* Open panel only via "+ New meeting" button */
            }}
          />
        )}
      </div>

      {showPanel ? (
        <NewMeetingPanel
          selectedDate={selectedDate}
          prefillStart={prefillStart}
          prefillEnd={prefillEnd}
          members={members}
          currentUserId={currentUserId}
          defaultAttendeeIds={defaultAttendeeIds}
          meetingsByMemberId={
            Object.keys(meetingsByMemberId).length
              ? meetingsByMemberId
              : selected
                ? { [selected.id]: meetings }
                : {}
          }
          submitting={submitting}
          onClose={closePanel}
          onRangeChange={handleDraftRangeChange}
          onSubmit={(draft) => void handleSubmit(draft)}
        />
      ) : null}

      {panelOpen && isMobile ? (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/50">
          <NewMeetingPanel
            selectedDate={selectedDate}
            prefillStart={prefillStart}
            prefillEnd={prefillEnd}
            members={members}
            currentUserId={currentUserId}
            defaultAttendeeIds={defaultAttendeeIds}
            meetingsByMemberId={
              Object.keys(meetingsByMemberId).length
                ? meetingsByMemberId
                : selected
                  ? { [selected.id]: meetings }
                  : {}
            }
            submitting={submitting}
            onClose={closePanel}
            onRangeChange={handleDraftRangeChange}
            onSubmit={(draft) => void handleSubmit(draft)}
          />
        </div>
      ) : null}
    </div>
  );
}
